...
public class Reader{
   ...
   public Reader(Scanner input){
      ...
      while(s.hasNext()){
         ...
         if(temp.substring(0,1).equals("<") && temp.substring(temp.length()-1).equals(">")){ // if the line indicates subject
            Subject s = new Subject(temp.substring(1,temp.length()-1));
            categories.add(s); // adds subject
         } else {
            if(temp.substring(0,1).equals("*") && temp.substring(temp.length()-1).equals("*")){ // if the line indicates topic
